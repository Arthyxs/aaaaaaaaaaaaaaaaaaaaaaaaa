/*
 * ===================================================================
 * Painel de Inteligência Financeira v4.5 - COMPLETO
 * ===================================================================
 * HISTÓRICO DE MUDANÇAS (v4.5):
 * - Corrigido o bug na secção "Lançamentos Sem Departamento".
 * - A lista agora mostra corretamente o valor da coluna 'VALOR PAGO'
 * para facilitar a auditoria, enquanto o resto do painel
 * continua a usar 'VALOR DEPARTAMENTO'.
 * ===================================================================
 */

// --- VARIÁVEIS GLOBAIS ---
let fullDataset = [];
let pizzaChart = null; // Guarda a instância do nosso gráfico

// --- FUNÇÕES UTILITÁRIAS DE ALTA CONFIABILIDADE ---
...
function updateDashboardUI(dataToDisplay, filterName = null) {
    const COLS = {
        ...
        VALOR_PAGO: 'VALOR PAGO', // Coluna para a auditoria
        DATA_PAGO: 'DT. PAGO'
    };
    ...
    if (!filterName) {
        document.getElementById('pizzaChart').style.display = 'block';
        renderPizzaChart(totalsByDept);
    } else {
        document.getElementById('pizzaChart').style.display = 'none';
    }
    ...
    document.getElementById('total-geral-title').textContent = filterName ? `Total Depto. (${filterName})` : 'TOTAL GERAL DEPTO.';
    ...
    if (issues.length === 0) {
        ...
        issues.forEach(row => {
            // CORREÇÃO: Usa a coluna 'VALOR_PAGO' especificamente para esta lista.
            const valorPago = parseBrazilianCurrency(row[COLS.VALOR_PAGO]);
            issuesList.innerHTML += `<li><span>${row[COLS.NATUREZA] || 'Natureza não informada'}</span> <span>${formatCurrency(valorPago)}</span></li>`;
        });
    }
}
